...
/**
 * Collects TPS, RAM, player count every 5 seconds.
 * Keeps a 5-minute rolling history for the dashboard chart.
 */
public class ServerStats {

    private static final int HISTORY_POINTS = 60; // 60 × 5s = 5 min
    ...
    public void start() {
        // Run every 5 seconds on main thread to safely access Bukkit API
        task = Bukkit.getScheduler().runTaskTimer(plugin, this::collect, 20L, 100L);
    }
    ...
    private void collect() {
        // TPS — Paper exposes getTPS() returning double[] {1m, 5m, 15m}
    ...
    /** Build a JSON snapshot for the /api/stats endpoint and WS push. */
